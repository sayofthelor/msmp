...
class SoundFrontEnd
{
	...
	public function playMusic(Music:FlxSoundAsset, Volume:Float = 1, Looped:Bool = true, ?Group:FlxSoundGroup):Void
	{
		...
		music.volume = Volume;
		music.persist = true;
		(Group ?? defaultMusicGroup).add(music);
		music.play();
	}
	...
	function loadHelper(sound:FlxSound, Volume:Float, Group:FlxSoundGroup, AutoPlay:Bool = false):FlxSound
	{
		...
		(Group ?? defaultSoundGroup).add(sound);
		return sound;
	}
	...
	public function changeVolume(Amount:Float):Void
	{
		...
		if (soundTrayEnabled)
		{
			if (Amount > 0)
			{
				play(BackendAssets.sound(volumeUpSound), 0.5);
			}
			else
			{
				play(BackendAssets.sound(volumeDownSound), 0.5);
			}
		}
		showSoundTray();
	}
